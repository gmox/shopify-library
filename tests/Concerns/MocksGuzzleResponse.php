...
trait MocksGuzzleResponse
{
    ...
    public function createMockedGuzzleResponse($responseDataToUse = [])
    {
        ...
        $this->mockedGuzzleResponse->shouldReceive('getBody')
                                   ->andReturn(json_encode($this->mockResponseData))
                                   ->byDefault();

        $this->mockedGuzzleResponse->shouldReceive('getStatusCode')
                                   ->andReturn('200')
                                   ->byDefault();

        $this->mockedGuzzleResponse->shouldReceive('getHeader')
                                   ->with('X-Shopify-Shop-Api-Call-Limit')
                                   ->andReturn(['5/40'])
                                   ->byDefault();
    }
}
